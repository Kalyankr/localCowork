...
class TaskManager:
    ...
    def update_state(
        ...
        self._emit_event(task, "state_change", {
            "old_state": old_state.value,
            "new_state": new_state.value,
        })
        
        # Persist on terminal states
        if new_state in {TaskState.COMPLETED, TaskState.FAILED, TaskState.REJECTED, TaskState.CANCELLED}:
            self._save_history()
    ...
    def update_step_progress(
        ...
        self._emit_event(task, "step_progress", {
            "step_id": step_id,
            "status": status,
            "current": current,
            "total": total,
    ...
    def set_step_result(
        ...
        self._emit_event(task, "step_complete", {
            "step_id": step_id,
            "result": result,
        })
