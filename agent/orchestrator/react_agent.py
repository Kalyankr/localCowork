...
ProgressCallback = Callable[[int, str, str, Optional[str]], None]  # iteration, status, thought, action
...
class ReActAgent:
                ...
                if iteration == 1:
                    ...
                    if last_step.result:
                        current_obs = Observation(
                            source="tool",
                            content=self._format_result(last_step.result)
                        )
                    else:
                        current_obs = Observation(
                            source="error",
                            content="No result from previous action"
                        )
    ...
    async def _think(
        ...
        try:
            ...
            thought = Thought(
                reasoning=response.get("thought", response.get("reasoning", "No reasoning provided")),
    ...
    def _build_history(self, state: AgentState) -> str:
        ...
        for step in state.steps[-5:]:  # Keep last 5 steps for context
            ...
            if step.result:
                ...
                output_preview = str(step.result.output)[:100] if step.result.output else ""
...
"""
    ...
    def _summarize_steps(self, state: AgentState) -> str:
        ...
        for step in state.steps:
            action_str = f"{step.action.tool}({step.action.description})" if step.action else "no action"
